document.addEventListener("DOMContentLoaded", function () {
    ...
    // xử lý article đầu trang
    function handleArticleTopPage() {
        const slidesArticlesContainer = document.querySelector('.js__slidesArticlesContainer');
        if (!slidesArticlesContainer) return;

        const articlesItem = slidesArticlesContainer.querySelectorAll('.js__slideArticleItem');
        if (articlesItem.length === 0) return;
        ...
        function updateActive(index) {
            // Xóa class active của item cũ
            articlesItem[currentIndex].classList.remove('active');
            
            // Cập nhật chỉ số mới
            currentIndex = index;
            
            // Thêm class active cho item mới
            articlesItem[currentIndex].classList.add('active');

            slidesArticlesContainer.className = slidesArticlesContainer.className
                .replace(/\bitem-\d+\b/g, '')
                .trim();
            slidesArticlesContainer.classList.add(`item-${index}`);
        }

        function nextSlide() {
            let nextIndex = (currentIndex + 1) % articlesItem.length;
        ...
        articlesItem.forEach((item, index) => {
        ...
        // Khi rời chuột khỏi vùng container chung thì chạy lại
        slidesArticlesContainer.addEventListener('mouseleave', startAutoPlay);
